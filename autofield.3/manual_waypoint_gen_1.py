...
def generate_waypoints(line_length_m: float,
                       ...
                       fix_2: tuple) -> list:
    """
    Generate a straight-line path whose direction is defined by two GPS fixes.

    Parameters
    ----------
    line_length_m : float
        Distance in metres from wp_1 to wp_2.
    fix_1 : (easting, northing)
        GPS fix recorded at the rover's start position.
    fix_2 : (easting, northing)
        GPS fix recorded after nudging the rover forward along the
        intended heading. The vector fix_1 → fix_2 defines the direction.
    ...
    Notes
    -----
    wp_1 is anchored at fix_1.
    wp_2 is fix_1 + line_length_m projected along the fix_1→fix_2 heading.
    The nudge distance itself does not matter — only the direction counts.

    Future extension
    ----------------
    For a rectangle, add width_m parameter and generate 4 corners using
    the perpendicular unit vector:
        perp = (-un, ue)   # 90° CCW from heading
        wp_3 = (wp_2[0] + perp[0] * width_m, wp_2[1] + perp[1] * width_m)
        wp_4 = (wp_1[0] + perp[0] * width_m, wp_1[1] + perp[1] * width_m)
    """
    de = fix_2[0] - fix_1[0]
    dn = fix_2[1] - fix_1[1]
    ...
    if dist < 0.05:
        raise RuntimeError(
            f"Nudge fixes are only {dist:.3f} m apart — too close to "
            "determine a reliable heading. Move the rover further forward."
        )

    # Unit heading vector along fix_1 → fix_2
    ue, un = de / dist, dn / dist
    heading_deg = math.degrees(math.atan2(un, ue))
    print(f"[WaypointGen] Heading derived: {heading_deg:.1f}° "
          f"(CCW from East — 0°=East, 90°=North)")

    wp_1 = fix_1
    wp_2 = (fix_1[0] + ue * line_length_m,
             fix_1[1] + un * line_length_m)
    return [wp_1, wp_2]
...
def run_startup(gnss=None) -> list:
    ...
    Full startup sequence:
      1a. Record fix_1 at the rover's start position
      1b. Nudge the rover forward; record fix_2 to establish heading
      2.  Prompt user for line length
      3.  Project wp_2 along the fix_1→fix_2 heading and return waypoints
    ...
    # ------------------------------------------------------------------
    # Step 1a: fix_1 at the start position
    # ------------------------------------------------------------------
    print("\nStep 1a: Place the rover at the START of your intended line.")
    input("         Press ENTER to record fix_1 ...\n")

    if gnss is not None:
        fix_1 = gnss.get_position()
        print(f"[GNSS] fix_1: E={fix_1[0]:.3f}  N={fix_1[1]:.3f}")
    else:
        fix_1 = read_averaged_fix(GNSS_PORT, GNSS_BAUD, NUM_FIX_AVERAGE)

    # ------------------------------------------------------------------
    # Step 1b: nudge rover forward to lock in heading
    # ------------------------------------------------------------------
    print("\nStep 1b: Drive/push the rover forward 2–5 m along your")
    print("         intended line direction, then stop.")
    input("         Press ENTER to record fix_2 (heading reference) ...\n")

    if gnss is not None:
        fix_2 = gnss.get_position()
        print(f"[GNSS] fix_2: E={fix_2[0]:.3f}  N={fix_2[1]:.3f}")
    else:
        fix_2 = read_averaged_fix(GNSS_PORT, GNSS_BAUD, NUM_FIX_AVERAGE)

    # Sanity-check nudge distance before asking for line length
    de = fix_2[0] - fix_1[0]
    dn = fix_2[1] - fix_1[1]
    nudge_dist = math.sqrt(de ** 2 + dn ** 2)
    print(f"[GNSS] Nudge distance: {nudge_dist:.3f} m")
    if nudge_dist < 0.5:
        raise RuntimeError(
            f"Nudge distance is only {nudge_dist:.3f} m — too small for a "
            "reliable heading. Move the rover further forward and retry."
        )
    ...
    waypoints = generate_waypoints(line_length, fix_1, fix_2)
