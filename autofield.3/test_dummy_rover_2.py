...
def dummy_startup(gnss: DummyGNSS) -> list:
    """
    Collect fix_1 and fix_2 from the dummy GNSS to establish heading,
    then ask for a line length.
    Returns a waypoint list identical in format to run_startup().
    """
    from manual_waypoint_gen import generate_waypoints

    print("\n" + "=" * 52)
    print("  Dummy Rover — Waypoint Setup")
    print("=" * 52)

    # Step 1a: fix_1 at start position
    print("\nStep 1a: Stand at your intended START position.")
    input("         Press ENTER to record fix_1 ...\n")
    fix_1 = gnss.get_position()
    print(f"  fix_1 recorded: E={fix_1[0]:.3f}  N={fix_1[1]:.3f}")

    # Step 1b: nudge forward to lock in heading
    print("\nStep 1b: Walk 2–5 m forward along your intended line direction.")
    input("         Press ENTER to record fix_2 (heading reference) ...\n")
    fix_2 = gnss.get_position()
    print(f"  fix_2 recorded: E={fix_2[0]:.3f}  N={fix_2[1]:.3f}")
    ...
    if nudge_dist < 0.5:
        raise RuntimeError(
            ...
            "reliable heading. Move further forward and retry."
        )
    ...
    print(f"\n  wp_1: E={waypoints[0][0]:.3f}  N={waypoints[0][1]:.3f}")
    print(f"  wp_2: E={waypoints[1][0]:.3f}  N={waypoints[1][1]:.3f}  "
          f"({line_length} m along derived heading)")
    return waypoints
...
def quick_run():
    ...
    # --- Waypoint setup ---
    print("\n  Point the rover at the START of your intended run.")
    input("  Press ENTER to record fix_1 from the current GPS fix ...\n")

    fix_1 = gnss.get_position()
    print(f"  fix_1: E={fix_1[0]:.3f}  N={fix_1[1]:.3f}")
    ...
    print("\n  Drive/push the rover forward 2–5 m along your intended line direction.")
    input("  Press ENTER to record fix_2 (heading reference) ...\n")

    fix_2 = gnss.get_position()
    print(f"  fix_2: E={fix_2[0]:.3f}  N={fix_2[1]:.3f}")
    ...
    print(f"  Nudge distance: {nudge_dist:.3f} m")
    if nudge_dist < 0.5:
        print(f"[QuickRun] ERROR: Nudge distance is only {nudge_dist:.3f} m — too small.")
        _cleanup(ser, gnss)
        sys.exit(1)
